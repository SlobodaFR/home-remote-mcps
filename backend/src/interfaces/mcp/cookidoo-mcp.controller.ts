...
import { CookidooDataGateway } from '../../application/mcp/cookidoo-data-gateway';
import { Public } from '../http/decorators/public.decorator';
import {
  ApiKeyGuard,
  McpAuthenticatedRequest,
} from '../http/guards/api-key.guard';
import { registerCookidooTools } from './cookidoo-tools';
...
@Controller('mcp/cookidoo')
export class CookidooMcpController {
  constructor(private readonly cookidooDataGateway: CookidooDataGateway) {}
  ...
  async handle(
    ...
    const server = new McpServer({
      name: 'home-remote-mcps-cookidoo',
      version: '1.0.0',
    });
    registerCookidooTools(server, this.cookidooDataGateway, req.mcpUserId);
