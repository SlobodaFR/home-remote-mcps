...
/** Talks to home-auth over HTTP (OAuth2 Authorization Code, confidential client). */
@Injectable()
export class HttpOAuthClient extends OAuthClient {
  ...
  async fetchUserInfo(accessToken: string): Promise<UserProfile> {
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to fetch user info: ${response.status.toString()}`,
      );
    }
  ...
  private async requestToken(
    ...
    if (!response.ok) {
      throw new Error(`Token request failed: ${response.status.toString()}`);
    }
