...
Garmin/Home Assistant/YouTube credential becomes permanently unusable and every user has to
reconnect.
...
shown three URLs — one per connected service (Garmin, Home Assistant, YouTube) — **shown only
once**. Copy the ones you need now.

See [`connecting-claude.md`](./connecting-claude.md) for how to add these URLs as custom
connectors in Claude.

Revoking a key immediately invalidates all three of its URLs.
